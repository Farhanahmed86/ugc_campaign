<template>
    <div class="panel">
        <div class="panel-heading">
            <h1 class="panel-title">General ledger</h1>
            <hr>
...
<div class="row">
        ...
        <div class="panel-body" v-if="show">
            <table id="print" class="table  table-bordered" style="color:#212F3D ">
                ...
                <tbody  style="cursor: pointer ; " >
                    <tr  v-for="item , index in model" :key="index" @click="detailsPage(item)">
              

                        <td class="w-1 ">{{index+1}}</td>
                        <td class="w-1 ">{{item.voucher ? item.voucher.created_at : ''}}</td>
                        <td class="w-1 ">{{item.voucher_no ? item.voucher_no : ''}}</td>
                        <td class="w-3 ">{{item ? item.account_code : ''}}</td>
                        <td class="w-3 ">{{item ? item.account : ''}}|{{item ? item.account_type : ''}}</td>
                        <td class="w-3 ">{{item.accounts ? item.accounts.account_type : ''}}</td>
                        <td class="w-3 ">{{item.voucher ? item.voucher.description : ''}}</td>
                        <td class="w-3 ">{{item.debit ? item.debit : '0.00'}}.00</td>


                        <td class="w-3">{{item.credit ? item.credit : '0.00'}}.00</td>
...
<!-- <div>Balance: {{ computeDebitCreditDiff(account.transactions) }}</div> -->
      ...
      <div class="document" v-if="one">
                ...
                <div class="document-body">
                  <hr>
                  <div class="row">
                  <strong class="col-6">Opening Balance: {{ amm }}</strong>
                  <strong class="col-6 text-right">Carry Forward Balance: {{ bal }}</strong>
                  ...
                  <hr>
                    <table class="table document-table">
                        ...
                        <tbody>
                            <tr v-for="item in ledg" :key="item.id">
...
groupVouchersByAccount(account) {
    return account.accounts;
            ...
            detailsPage(item) {
              // console.log(this.params.todate);
              // const routeParams = {
              //     accountTypeId: item.account_type_id,
              //     toDate: this.params
              //   };
                const accountTypeId = item.account_type_id;
                 const toDate = this.params;
                 let params = "?q=";
                if (this.form.account_id != null) {
                    params += "&account_id=" + this.form.account_id;
                }
                ...
                get(`/api/accounts_ledger/${accountTypeId}/${params}`)
                .then((res ) => {
                  Vue.set(this.$data, 'ledg', res.data.data)
                  Vue.set(this.$data, 'amm', res.data.amount)
                  Vue.set(this.$data, 'bal', res.data.balance)


                  // this.amm = res.data.amount;
                  // this.bal = res.data.balance;
                ...
                // this.$router.push(`/ledgers/${item.account_type_id}`)
            },
            setData(res) {
